...
static int32_t get_saved_slider_value(void) {
    if (saved_focused_index >= 1 && saved_focused_index <= 10) {
        return 11 - saved_focused_index;
    }
    return 10;
}
...
void update_slider_timer(lv_timer_t *timer)
{
    ...
    if (!charge_icon_active && filtered_bat >= 4.18f)
    {
        charge_icon_active = true;
    }
    else if (charge_icon_active && filtered_bat <= 4.05f)
    {
