...
#define SHORT_PRESS_THRESHOLD 40                                      // 定义短按时间阈值（防误触）
#define DOUBLE_CLICK_THRESHOLD 500                                    // 定义双击时间阈值
#define KEY_DEBOUNCE_MS 25                                            // 按键去抖时间
...
void keypad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) // 按键扫描函数（PUSH_PIN 支持双击返回）
{
  ...
  static uint8_t raw_last_state = 0;
  static uint8_t stable_key_state = 0;
  static uint32_t key_state_change_time = 0;
  uint32_t now = millis();
  uint8_t key_state = 0;
  ...
  // 按键去抖：只有状态稳定一段时间后才生效
  if (raw_key_state != raw_last_state)
  {
    raw_last_state = raw_key_state;
    key_state_change_time = now;
  }
  if ((now - key_state_change_time) >= KEY_DEBOUNCE_MS)
  {
    stable_key_state = raw_key_state;
  }
  key_state = stable_key_state;

  if (key_state == 3 && !push_pressed)
  {
    push_pressed = true;
    push_start_time = now;
  }
